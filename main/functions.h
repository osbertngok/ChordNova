...
// One of our goals is to expand 'm_notes' to a size of 'm_max' by adding some m_notes from itself.
// We can create a vector 'expansion' (*) with size 'm_max' with elements from {0, 1, ..., m_notes.size() - 1}
// (sorted in ascending order), then the desired expansion is simply { m_notes[expansion[i]] }.
// 'expansion_indexes[min][max][i]' is the 'i'th alternative of expansion (*) for 'm_notes.size() = min' and 'm_max = max'.
extern int note_pos[ET_SIZE];
...
// Contains 'set_id' of all 12 transpositions of all 'pitch_class_set's in the chord database file.
...
extern int find_root(const vector<int> &);
