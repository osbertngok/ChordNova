...
int note_pos[ET_SIZE] = {1, 9, 9, 3, 3, 11, 11, 5, 13, 13, 7, 7};
...
// Reads the chord database.
{
  ...
  do {
    ...
    vector<int> omit_choice;  // contains notes from 'pitch_class_set' that can be omitted
    for (int i = 0; i < s_size; ++i) {
      int diff = (note_set[i] - root) % ET_SIZE;
...
// 'rec' will contain 'set_id' for all 12 transpositions of 'pitch_class_set'.
{
  for (int j = 0; j < ET_SIZE; ++j) {
    int val = 0;
    for (int i = 0; i < (int) note_set.size(); ++i)
      val += (1 << ((note_set[i] + j) % ET_SIZE));
    rec.push_back(val);
  }
...
// We assume that 'set' is sorted in ascending order.
{
  ...
  for (int i = 0; i < len; ++i) {
    ...
    for (int j = len - 1; j > 0; --j) {
      ...
      if (interval < 0) interval += ET_SIZE;
...
int find_root(const vector<int> &notes)
...
// Notes should be sorted in ascending order.
{
  ...
  for (int i = 0; i < t_size; ++i)
    for (int j = i + 1; j < t_size; ++j) {
      int interval = (notes[j] - notes[i]) % ET_SIZE;
...
bool smaller(const int &num1, const int &num2) { return num1 < num2; }
